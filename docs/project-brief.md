...
- The user prefers trying the Antigravity managed agent first, reporting higher daily and per-minute API quotas for it than for direct Gemini; test Gemini direct if Antigravity does not work.
...
User preference: try the Antigravity managed agent first because the user reports higher daily and per-minute token quotas on their API access, then try direct Gemini if it does not work. This account-specific quota difference is user-reported and remains to be confirmed by live usage. The Antigravity API uses the Interactions endpoint and agent ID `antigravity-preview-09-2026`; it is not a Gemini model ID for `GOOGLE_MODEL`. The agent currently defaults to Gemini 3.8 Flash under its managed harness, with a separate supported-model setting. The app disables agent tools for chat and keeps each turn in backend-managed interaction state. See the [managed agents quickstart](https://ai.google.dev/gemini-api/docs/managed-agents-quickstart) and [streaming guide](https://ai.google.dev/gemini-api/docs/streaming). See [Gemini model guide](https://ai.google.dev/gemini-api/docs/latest-model).
...
- Live Antigravity access/quota and Gemini fallback verification; exact entry-code mechanism.
