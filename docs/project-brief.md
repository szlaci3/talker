...
- Target standard desktop browsers; phones are out of scope. When WebMCP is unavailable, display a notice and continue with the best available fallbacks. Native WebMCP is not required in every browser; desktop browser/OS coverage is partial, unknown, and accepted.
...
- Voice output uses `edge-tts`, following the voice experience in the user's Reciter website: browser Daniel during warmup, then preferably Microsoft Brian Multilingual Online (Natural) - English (United States). The live catalogue and synthesis verified `en-US-BrianMultilingualNeural`; browser playback and the Daniel fallback still need post-deploy user checks. Later voice input must show evolving transcription while the user speaks, without waiting for dictation to finish before displaying text.
...
Voice policy: prefer Microsoft Brian Multilingual Online (Natural) - English (United States) immediately when available; Daniel is a temporary warmup/fallback voice, not a compulsory first utterance. Reciter selects browser-provided Daniel (British English, preferred pitch 1.4); it is not an `edge-tts` voice identifier and is not available on every device. Reciter's current remote default is `en-GB-SoniaNeural`; this project's requested remote voice is Brian. Resolve Brian from the service catalogue and do not silently substitute a similar remote voice. If Daniel is absent, report the browser default voice accurately.
...
- Brian's catalogue entry and synthesis are verified locally as `en-US-BrianMultilingualNeural`; actual browser playback and Daniel availability still need user verification after deployment.
...
Next step: deploy and manually verify milestone 3 playback and voice handoff after a cold start; see [implementation-status.md](implementation-status.md). Native WebMCP invocation still needs a supported browser/agent.
