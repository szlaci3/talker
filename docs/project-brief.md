...
- Low-cost Google API access is preferred; user proposed the Antigravity agent based on Gemini 3.8 and its free quota.
...
Flow: Netlify browser chat -> Render API -> Google API -> streamed answer/tool request -> browser validates and executes a known UI action -> tool result returns to the model when needed. Speech: completed text segments -> Render `edge-tts` -> browser audio playback. Configure allowed frontend origins and a working session transport; CORS is not authentication. Direct cross-site calls versus a same-origin proxy remains an implementation choice to validate for streaming and cookies.

Keep the Google key in Render server environment variables, never in the browser bundle, browser storage, or a VITE-prefixed variable. Backend responsibilities: entry/session validation, request limits for chat and TTS, input/context/output bounds, provider calls, timeouts, cancellation, and useful quota errors. Conversation persistence is undecided; a proposed initial default is tab-session memory with no conversation database. Shared usage counters require platform enforcement or durable storage rather than restart-sensitive process memory.
...
Recommend direct Gemini 3.8 Flash for the first chat/tool loop, keeping model/provider configuration replaceable. The managed agent's sandbox is unnecessary for changing local display preferences, and does not automatically access the visitor's page. Retain Antigravity as the user's preferred candidate for evaluation if its managed capabilities are desired. Prefer asynchronous start/status requests for long agent jobs. See [Gemini model guide](https://ai.google.dev/gemini-api/docs/latest-model).
...
Next step: when the user requests implementation in a new task, follow [implementation-handover.md](implementation-handover.md), beginning with the text MVP. Reciter's implementation has been inspected; no live integration has been verified in this project.
